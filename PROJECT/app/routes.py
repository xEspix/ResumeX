...
from app.genAI import screenResume
import tempfile
import os
...
def uploadForm():
    ...
    if form.validate_on_submit():
        ...
        output_dict=screenResume(text)
        print(output_dict)
        with app.app_context():
            resume_data=Resume(username=form.username.data,
                               name=output_dict['Name'],
                               email=output_dict['Email'],
                               phone_no=output_dict['Phone_No'],
                               location=output_dict['Location'],
                               degree=output_dict['Degree'],
                               cllg_name=output_dict['College_Name'],
                               gpa=output_dict['GPA'],
                               grad_year=output_dict['Graduation_Year'],
                               skills=output_dict['Skills'],
                               courses=output_dict['Certified_Courses'],
                               designation=output_dict['Designation'],
                               companies=output_dict['Companies_Worked'],
                               experience=output_dict['Years_of_Experience'],
                               achievements=output_dict['Achievements'],
                               awards=output_dict['Awards'],
                               linkedin=output_dict['LinkedIn_Profile'],
                               github=output_dict['GITHUB_Profile'],
                               projects=output_dict['Projects'],
                               layout_score=output_dict['Layout_Score'],
                               layout_tips=output_dict['Layout_Tips'],
                               smart_project_score=output_dict['Smart_Project_Score'],
                               project_tips=output_dict['Project_Tips'],
                               numerical_metrics=output_dict['Numerical_Metrics'],
                               top_skillset=output_dict['Top_Skillset'],
                               rate=output_dict['Rate'])
